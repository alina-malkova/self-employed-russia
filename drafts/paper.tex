...
\begin{abstract}
I measure the welfare cost of labor informality through consumption volatility rather than wages or productivity. Using longitudinal data from the Russian Longitudinal Monitoring Survey (2010--2023), I show that informal wage workers face significantly worse consumption insurance than formal workers: a one-standard-deviation income shock passes through to consumption at nearly twice the rate for informal workers ($\beta = 0.116$) compared to formal workers ($\beta = 0.064$). To identify whether access to formal self-employment status improves consumption smoothing, I exploit the staggered rollout of Russia's \textit{nalog na professionalnyi dokhod} (NPD) tax regime across regions in 2019--2020. After NPD adoption, the income--consumption pass-through for informal wage workers drops from 0.128 to 0.068, converging to the level of formal workers. These results suggest that the welfare cost of informality operates substantially through the consumption insurance channel---a margin that standard wage-based and structural approaches underestimate.

\medskip\noindent
\textbf{JEL codes:} J46, O17, D12, H26

\medskip\noindent
\textbf{Keywords:} informality, consumption insurance, welfare, Russia, self-employment, tax reform
\end{abstract}
...
Labor informality is pervasive in developing and transition economies, yet its welfare consequences remain poorly understood. A large literature documents the size of the informal sector, its determinants, and the wage gap between formal and informal workers.\footnote{See \citet{LaPortaShleifer2014} and \citet{Ulyssea2020} for surveys.} The dominant approaches to measuring the welfare cost of informality---structural firm-side models \citep{Ulyssea2018} and search-and-matching frameworks \citep{BoeriGaribaldi2007, CharlotMalherbet2013, Meghir2015}---quantify welfare through wages, productivity, or the value of matching. These approaches capture important margins. But they miss a channel that may be equally or more consequential for workers' well-being: the ability to smooth consumption over time.

An informal worker who earns the same wage as a formal worker is not equally well off. Formal employment in most countries provides access to social insurance---pensions, sick leave, unemployment benefits, maternity payments---that buffers household consumption against income shocks. Informal workers, by definition excluded from these programs, must absorb income fluctuations through savings, family transfers, or reduced consumption. The welfare cost of this exposure is not captured by comparing wages across sectors, nor by structural models that equate welfare with aggregate output.

This paper measures the welfare cost of informality directly through consumption volatility. Using fourteen waves of the Russian Longitudinal Monitoring Survey (RLMS-HSE, 2010--2023), I construct a panel of 43,280 individuals with matched household-level consumption and individual-level employment data. Following the consumption insurance test of \citet{Townsend1994}, I regress consumption growth ($\Delta \ln c$) on income growth ($\Delta \ln w$) separately by employment type. Under full consumption insurance, income shocks should not pass through to consumption; the pass-through coefficient $\beta$ measures the degree of insurance failure.

I find that informal wage workers face substantially worse consumption insurance than formal workers. The pass-through of income shocks to consumption is $\beta = 0.116$ for informal wage workers, compared to $\beta = 0.064$ for formal workers---nearly twice the rate. Self-employed workers fall between the two groups at $\beta = 0.085$. The consumption growth variance for informal wage workers is 12.4\% higher than for formal workers (Levene's test $p = 0.001$). These cross-sectional differences persist across all years in the sample and are robust to controlling for age, gender, and year fixed effects.

To move beyond cross-sectional comparisons, I exploit a natural experiment: the staggered introduction of Russia's \textit{nalog na professionalnyi dokhod} (NPD, ``tax on professional income''), a simplified self-employment tax regime launched in January 2019. The NPD was rolled out in waves---four pilot regions in 2019, nineteen additional regions in January 2020, and all remaining regions by October 2020---creating plausibly exogenous variation in access to a formal self-employment status across regions and time. The reform lowered the barrier to formalization: instead of registering as an individual entrepreneur with accounting obligations and social insurance contributions, workers could register through a smartphone application and pay a flat 4--6\% tax on revenue with no additional reporting.

I estimate a triple-difference specification: the change in income--consumption pass-through for informal relative to formal workers, in treated relative to not-yet-treated regions, after NPD adoption. The key coefficient $\hat{\beta}_4 = -0.042$ is negative, as predicted by the theory---NPD availability reduces the consumption vulnerability of informal workers. The by-type analysis is most striking: the pass-through for informal wage workers falls from 0.128 pre-NPD to 0.068 post-NPD, converging almost exactly to the formal worker level (0.068). The event study shows the gap between informal and formal pass-through coefficients present in the pre-period closes around the time of NPD adoption.

These results are economically meaningful but statistically imprecise in the triple-difference specification ($p = 0.25$), reflecting the inherent power limitations of the RLMS: the survey covers 32 federal subjects with approximately 4,100 informal wage workers in the estimation sample. The descriptive patterns are, however, highly robust and statistically significant in pooled comparisons.

This paper makes three contributions. First, I document a consumption insurance channel of informality that is distinct from wage and productivity differences. The standard Townsend test provides a transparent, reduced-form welfare measure that does not require the strong structural assumptions of \citet{Ulyssea2018} or the high-frequency flow data needed by search models. Second, I provide the first evidence on whether access to simplified formal self-employment status improves consumption smoothing for informal workers, using the NPD rollout as a natural experiment. Third, I contribute to the literature on Russia's large informal sector \citep{GimpelsonKapeliushnikov2014, LehmannZaiceva2013} by documenting a welfare cost that is not visible in the wage distribution---informal and formal workers earn comparable wages, but informal workers bear significantly more consumption risk.

The remainder of the paper proceeds as follows. Section~\ref{sec:literature} reviews the related literature. Section~\ref{sec:background} describes the institutional setting and the NPD reform. Section~\ref{sec:data} presents the data. Section~\ref{sec:empirical} describes the empirical strategy. Section~\ref{sec:results} presents results. Section~\ref{sec:conclusion} concludes.
...
A candid limitation of this analysis is statistical power. The estimation sample contains 73,435 observations, but the identifying variation comes from 32 oblasts and approximately 4,100 informal wage workers with both consumption and income growth data. With 32 clusters for inference, standard asymptotic cluster-robust standard errors may over-reject \citep{CallawayS2021}. I address this concern by reporting results both pooled and by cohort, and by emphasizing the economic magnitude and consistency of the estimates alongside their statistical significance.
...
This paper measures the welfare cost of labor informality through a channel that standard approaches miss: the ability to smooth consumption over income shocks. Using fourteen waves of the RLMS-HSE and the staggered rollout of Russia's NPD self-employment tax regime, I document three findings.

First, informal wage workers face substantially worse consumption insurance than formal workers. The pass-through of income shocks to consumption is 0.116 for informal wage workers versus 0.064 for formal workers---nearly twice the rate. This gap is not visible in wages: informal and formal workers earn comparable mean wages, and the self-employed earn the highest wages of the three groups. The welfare cost of informality operates through the insurance channel, not the wage channel.

Second, the introduction of the NPD---a simplified tax registration that provides legal status without social insurance---is associated with a sharp improvement in consumption insurance for informal wage workers. The income--consumption pass-through for informal wage workers drops from 0.128 pre-NPD to 0.068 post-NPD, converging exactly to the formal worker level. This convergence is concentrated entirely in informal wage workers; formal workers and the self-employed show no change. The triple-difference estimate of $-0.042$ has the predicted sign and closes 88\% of the pre-NPD insurance gap, though it is not statistically significant at conventional levels ($p = 0.25$) due to the limited number of clusters (32 oblasts) and the small informal sample.

Third, the NPD has zero effect on RLMS-measured informality rates, ruling out the possibility that the insurance improvement reflects mechanical reclassification of workers. Whatever channel drives the improvement---access to formal credit, the ability to contract with firms requiring documented suppliers, reduced risk of fines, or the psychological security of legal status---it operates through behavioral changes among workers who continue to report informal employment by survey definitions.

These findings have implications for both research and policy. For the informality literature, the results demonstrate that wage-based and productivity-based welfare measures understate the cost of informality by ignoring the insurance channel. Two workers earning identical wages are not equally well off if one absorbs income shocks directly into consumption while the other is buffered by social insurance. The Townsend consumption insurance test provides a transparent, reduced-form alternative that requires neither structural assumptions nor high-frequency flow data. For the formalization policy literature, the results suggest that even partial formalization---tax registration without social insurance---can improve welfare through indirect insurance channels. This finding qualifies the standard view that formalization benefits workers primarily through direct access to social insurance programs.

Several limitations suggest directions for future work. The statistical imprecision of the triple-difference calls for replication with larger samples---either through additional survey waves of the RLMS or through administrative data that would increase the number of clusters and the informal sample size. The mechanisms through which tax-only formalization improves consumption insurance remain speculative; survey or administrative evidence on credit access, business contracting, and savings behavior among NPD registrants would sharpen the interpretation. Finally, the welfare calculation presented here captures only the insurance channel; a complete accounting of the costs of informality would integrate the insurance margin documented in this paper with the wage, productivity, and fiscal margins emphasized by the existing literature.
